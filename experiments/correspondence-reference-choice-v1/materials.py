...
def _session(group, cell, source, provenance, figures):
    ...
    source_occurrences = [j for j in range(len(history) - 11)
                          if _signature(history[j:j + 12, 2]) == _signature(source[:, 2])]
    if source_occurrences != [2]:
        raise ValueError("Unintended complete source-equivalent 12-event occurrence in history")
    if cell in ("inventory_foil", "near_foil"):
        if any(_signature(history[j:j + 12, 2]) == _signature(query[:, 2])
               for j in range(len(history) - 11)):
            raise ValueError("A complete foil interval sequence already occurs in history")
