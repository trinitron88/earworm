...
def main(split):
    d=HERE/'data'/split;cfg=json.loads((d/'config_used.json').read_text());truth={r['episode']:r for r in map(json.loads,read_lines(d/'truth.jsonl.gz'))};cache=list(map(json.loads,read_lines(d/'cache.jsonl.gz')));waves={r['episode']:r for r in map(json.loads,read_lines(d/'wave_archive.jsonl.gz'))};records=list(map(json.loads,read_lines(d/'events.jsonl.gz')));checks={};acoustic_count=0;acoustic_error=0.;endpoint_cases=[];previous='0'*64;by=defaultdict(list)
    ...
    for x in cache:
        ...
        for key,frames in pred.get('lattice',{}).items():
            ...
            for z in frames:
                ...
                # Saved-only reconstruction: energy and spectrum have different supports.
                samples=np.frombuffer(raw[z['start_sample']*4:z['end_sample']*4],dtype='<f4').astype(np.float64)
                rms=float(np.sqrt(np.mean(samples*samples)))
                power=np.abs(np.fft.rfft(samples*np.hanning(len(samples))))**2
                bands=np.asarray([float(part.sum()) for part in np.array_split(power,8)])
                total=float(bands.sum());denominator=max(total,1e-20)
                shape=bands/denominator
                assert len(z['spectral_shape'])==8
                error=max(abs(rms-z['observation']['rms']),max(abs(float(a)-b) for a,b in zip(shape,z['spectral_shape'])))
                assert error<=1e-12
                assert abs(sum(z['spectral_shape'])-total/denominator)<=1e-12
                acoustic_count+=1;acoustic_error=max(acoustic_error,error)
                if total==0 and rms>=1e-8:
                    endpoint_cases.append(dict(episode=eid,lattice=key,start_sample=z['start_sample'],end_sample=z['end_sample'],wave_sha256=z['observation']['wave_sha256'],rms=rms,total_band_power=total,denominator=denominator,spectral_shape=z['spectral_shape'],nonzero_indices=np.flatnonzero(samples).tolist()))
    ...
    for eid,t in truth.items():
        ...
        if t['condition']=='swap':
            for v in [0,1]:
                sx=truth[eid[:-1]+str(v)];ix=truth[f"{t['group']}/unchanged/intact/{1-v}"];assert waves[sx['episode']]['prefix_b64']==waves[ix['episode']]['prefix_b64'] and sx['pitch_semitones']==ix['pitch_semitones']
