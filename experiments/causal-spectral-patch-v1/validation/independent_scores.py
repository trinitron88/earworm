...
truth={r['episode']:r for r in map(json.loads,read_lines(P/'data'/S/'truth.jsonl.gz'))};trials={(r['model'],r['group'],r['cell'],r['condition'],int(r['variant'])):r for r in csv.DictReader((P/'results'/S/'trials.csv').open())};count=0;error=0.
for x in map(json.loads,read_lines(P/'data'/S/'cache.jsonl.gz')):
 t=truth[x['episode']]
 for front,forecasts in {**x['operational']['forecasts'],**{'patch-'+k:v for k,v in x['operational']['patch_forecasts'].items()},'oracle-boundary':x['oracle_forecasts']}.items():
