    ...
    for cell in CELLS:
        ...
        for cond in (CONDITIONS if cell=='joint' else ['intact']):
          for variant in [0,1]:
            ...
            episodes.append(dict(group=g['group_id'],cell=cell,condition=cond,variant=variant,prefix=raw,bounds=bounds,probe_start=probe_start,probe_end=probe_end,reset_sample=reset,future=tone(target,3,True),alternative_future=tone(ends[1-binding],3,True),truth=dict(pitch_semitones=target,prefix_true_pitches=Q,outcome_intervals=[-d,d] if cond=='ambiguous' else [target-Q[-1]],outcome_probabilities=[.5,.5] if cond=='ambiguous' else [1.])))
