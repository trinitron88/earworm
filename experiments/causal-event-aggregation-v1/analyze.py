...
def main(split):
    ...
    for x in cache:
        t=truth[x['episode']];outputs={'map':x['operational']['forecasts'][cfg['selected_aggregation']],'mixture':x['operational']['forecasts'][cfg['selected_aggregation']+'_mixture'],'repaired':x['operational']['forecasts'][cfg['selected_support']],'streaming':x['operational']['forecasts'][selected],'fixed':x['operational']['forecasts']['fixed'],'oracle-boundary':x['oracle_forecasts']};evs={'map':x['operational']['events'][cfg['selected_aggregation']],'mixture':x['operational']['events'][cfg['selected_aggregation']+'_mixture'],'repaired':x['operational']['events'][cfg['selected_support']],'streaming':x['operational']['events'][selected],'fixed':x['operational']['events']['fixed'],'oracle-boundary':x['oracle_events']}
        for front,fs in outputs.items():
            for name,f in fs.items():
                model=front+'/'+name;preds[(model,t['group'],t['cell'],t['condition'],t['variant'])]=f;rows.append(dict(model=model,group=t['group'],cell=t['cell'],condition=t['condition'],variant=t['variant'],**measure(f,t),entropy=-sum(v*math.log(v) for v in absolute_distribution(f) if v>0),unknown_mass=absolute_distribution(f)[-1],confidence=max(absolute_distribution(f))))
    ...
    groups=sorted({r['group'] for r in rows});models=sorted({r['model'] for r in rows});draw=np.random.default_rng(2026092060).integers(0,len(groups),(5000,len(groups)))
    ...
    for r in gr:
        rs=bucket[tuple(r[k] for k in ['model','group','cell','condition'])]
        for metric in ['entropy','unknown_mass','confidence']:r[metric]=float(np.mean([x[metric] for x in rs]))
    ...
    for model,cell,cond in sorted({(r['model'],r['cell'],r['condition']) for r in rows}):
        ...
        for metric in ['accuracy','paired_success','log_loss','brier','entropy','unknown_mass','confidence']:
    ...
    reliability=[]
    for model,cell,cond in sorted({(r['model'],r['cell'],r['condition']) for r in rows}):
        rs=[r for r in rows if (r['model'],r['cell'],r['condition'])==(model,cell,cond)]
        for bin in range(10):
            z=[r for r in rs if min(9,int(r['confidence']*10))==bin]
            if z:reliability.append(dict(model=model,cell=cell,condition=cond,bin=bin,count=len(z),mean_confidence=float(np.mean([r['confidence'] for r in z])),accuracy=float(np.mean([r['accuracy'] for r in z]))))
    table(out/'calibration_bins.csv',reliability)
    candidates=[]
    for x in cache:
        t=truth[x['episode']]
        for e in x['operational']['events'][cfg['selected_aggregation']]:
            if e['state_pitch'] is None:continue
            q=e['distribution'];candidates.append(dict(group=t['group'],cell=t['cell'],condition=t['condition'],variant=t['variant'],window_count=q['window_count'],valid_count=q['valid_count'],candidate_count=len(q['candidates']),cluster_count=q['cluster_count'],concentration=q['concentration'],disagreement=q['disagreement'],unknown_mass=q['unknown_mass'],available=int(q['point'] is not None)))
    table(out/'candidate_trials.csv',candidates)
    cg=[]
    for g in groups:
        z=[r for r in candidates if r['group']==g];cg.append(dict(group=g,**{k:float(np.mean([r[k] for r in z])) for k in ['window_count','valid_count','candidate_count','cluster_count','concentration','disagreement','unknown_mass','available']}))
    table(out/'candidate_groups.csv',cg)
    ...
    for front in ['map','mixture','repaired','streaming','fixed','oracle-boundary']:
      for g in groups:
    ...
    for model in NAMES:
      for cell in CELLS:
        vals=vector('mixture/'+model,cell,'intact','accuracy')-vector('oracle-boundary/'+model,cell,'intact','accuracy');lo,hi=ci(vals,(.05/8,1-.05/8));contrasts.append(dict(model=model,cell=cell,repaired_minus_oracle=float(vals.mean()),lower_familywise95=lo,upper_familywise95=hi))
        if model=='reference':noninferior &=lo>=-.1
    table(out/'comparisons.csv',contrasts)
    primary='mixture/reference';ceiling='oracle-boundary/reference';oracle_pass=all(vector(ceiling,c,'intact','paired_success').mean()>=.8 for c in CELLS);stream_pass=all(vector(primary,c,'intact','paired_success').mean()>=.8 for c in CELLS)
    advantage=vector(primary,'joint','intact','accuracy')-vector('mixture/present','joint','intact','accuracy');lo,hi=ci(advantage)
    increases={m:float(vector(primary,'joint','intact',m).mean()-vector(ceiling,'joint','intact',m).mean()) for m in ['log_loss','brier']}
    ev=next(r for r in es if r['front']=='map');whole=next(r for r in es if r['front']=='streaming');ov=next(r for r in es if r['front']=='oracle-boundary');event_pass=ev['f1']>=.90 and ev['pitch_accuracy']>=.95 and ev['split_rate']<=.05 and ev['merge_rate']<=.05;latency=ev['max_true_end_latency_s']<=.160 and ev['max_inferred_end_latency_s']<=.160
    ...
    whole_gain=vector(primary,'joint','intact','accuracy')-vector('repaired/reference','joint','intact','accuracy');whole_lo,whole_hi=ci(whole_gain)
    valid=json.loads((HERE/'validation'/f'{split}.json').read_text())['all_pass']
    proper=increases['log_loss']<=.10 and increases['brier']<=.02
    adoption=oracle_pass and stream_pass and noninferior and lo>0 and whole_lo>0 and proper and event_pass and latency and controlpass[primary] and safety_pass
    # Material improvement over trim16 is the preregistered positive joint lower bound.
    # C takes precedence when measurement/safety fails or trim16 is not improved;
    # B requires those prerequisites plus calibration/ambiguity failure.
    if not valid:branch='invalid'
    elif not oracle_pass or not(event_pass and safety_pass and latency) or whole_lo<=0:branch='C'
    elif adoption:branch='A'
    elif not proper or not controlpass[primary]:branch='B'
    else:branch='inconclusive'
    decision=dict(branch=branch,validity_passed=valid,oracle_ceiling_pass=bool(oracle_pass),mixture_paired_success_pass=bool(stream_pass),noninferiority=bool(noninferior),joint_over_present_lower95=lo,joint_proper_score_increases=increases,event_gates=bool(event_pass),latency_pass=bool(latency),control_pass=controlpass[primary],safety_pass=bool(safety_pass),trim16_comparison=dict(joint_gain=float(whole_gain.mean()),lower95=whole_lo,upper95=whole_hi),next={'A':'After review PM may assign isolated-voice recognition/transformation milestone.','B':'Measurement candidate only; next bounded decision must concern uncertainty/readout, not boundaries.','C':'Reject aggregator, end crop/within-event aggregation micro-repairs; PM selects different event representation or concrete blocker.','inconclusive':'Preserve all outcomes; no forced selection or retest.','invalid':'Preserve invalid evidence and stop.'}[branch]);dump(out/'decision.json',decision);print(json.dumps(safe(decision)))
