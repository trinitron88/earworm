...
def run(split,config_path):
    ...
    if split=='heldout':
        for name,h in frozen['files'].items():
            blob=subprocess.check_output(['git','-C',str(HERE),'show','HEAD:experiments/controlled-motif-robustness-v1/'+name])
        ...
        for entry in entries:
            ...
            for ep in episodes:
                ...
                for name,f in pred['forecasts'].items():
                    ...
                    for interval,mass in zip(truth['outcome_intervals'],truth['outcome_probabilities']):
                        anchor=f['anchor_semitones']; offset=None if anchor is None else truth['prefix_true_pitches'][-1]+interval-anchor
                        k=49 if offset is None or not -24.5<offset<24.5 else int(round(offset))+24
                        y[k]+=mass
                    scoring[name]={'correct_within_tolerance':f['point_pitch_semitones'] is not None and abs(f['point_pitch_semitones']-truth['pitch_semitones'])<=.35,'log_loss':float(-sum(y[j]*np.log(probs[j]) if probs[j]>0 else float('inf') for j in np.flatnonzero(y))), 'brier':float(sum((probs-y)**2))}
