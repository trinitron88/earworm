...
def run(split,config_path):
    ...
    if split=='heldout':
        for name,h in frozen['files'].items():
            blob=subprocess.check_output(['git','-C',str(HERE),'show','HEAD:experiments/history-prediction-v1/'+name])
        ...
        for entry in entries:
            ...
            for family in ['basic','transfer']:
                for condition in ['intact','reset','removal','swap','shuffle','ambiguous']:
                    ...
                    balance.append({'group_id':entry['group_id'],'family':family,'condition':condition,'identical_multiset':True,'identical_final_two_history_and_probe':True,'blocks_per_prefix':15,'duration_s':1.8})
            for ep in episodes:
                ...
                assert pred['prefix_sha256']==digest(canonical(prefix_hashes).encode()) and pred['arrivals']==15 and pred['evictions']==0
                ...
                scoring={}
                for name,f in pred['forecasts'].items():
                    probs=np.array(f['probabilities']);y=np.zeros(50)
                    for interval,mass in zip(truth['outcome_intervals'],truth['outcome_probabilities']):y[int(round(interval))+24]+=mass
                    scoring[name]={'correct_within_tolerance':f['point_pitch_semitones'] is not None and abs(f['point_pitch_semitones']-truth['pitch_semitones'])<=.35,'log_loss':float(-sum(y*np.log(probs))), 'brier':float(sum((probs-y)**2))}
