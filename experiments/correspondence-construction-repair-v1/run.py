...
def readout_count():
    probes=ROOT/'qualification-additional.json'
    added=len(json.loads(probes.read_text())['calls']) if probes.exists() else 0
    return 29+added+len(list((ROOT/'outputs').glob('**/started.json')))

def resource_guard(reserve=300):
    stage=json.loads((ROOT/'stage.json').read_text())
    now=datetime.datetime.now(datetime.timezone.utc)
    elapsed=sum(((datetime.datetime.fromisoformat(v['end']) if v['end'] else now)-datetime.datetime.fromisoformat(v['start'])).total_seconds() for v in stage['active_intervals'])
    helper=stage.get('helper_charge_seconds',900.0)
    additional=elapsed+helper
    if additional+reserve>3400 or 3770.938094+additional+reserve>7200:
        raise RuntimeError('Cumulative compute budget requires checkpoint')
    calls=readout_count()
    if calls>=400:raise RuntimeError('Cumulative readout budget exhausted')
    old=ROOT.parent/'correspondence-reference-choice-v1'
    n=sum(p.stat().st_size for base in [ROOT,old] for p in base.rglob('*') if p.is_file() and '__pycache__' not in p.parts)
    if n>512*1024**2:raise RuntimeError('Cumulative new-artifact budget exceeded')
    return {'cumulative_calls':calls,'prior_conservative_compute_seconds':3770.938094,
      'active_worker_wall_seconds':elapsed,'parallel_helper_charge_seconds':helper,
      'additional_compute_seconds':additional,'cumulative_compute_seconds':3770.938094+additional,
      'publication_reserve_seconds':reserve,'new_artifact_bytes':n,'spend':0,
      'additional_compute_cap_seconds':3400,'cumulative_compute_cap_seconds':7200,
      'call_cap':400,'qualification_call_cap':32,'artifact_cap_bytes':512*1024**2}
...
def main():
    ...
    assert json.loads((ROOT/'preflight.json').read_text())['passed'], 'Complete packaging validation is required'
    ...
    for session in sessions:
        directory=base/session['path'];rows=loadnp(directory/'rows.npz');events=loadnp(directory/'exact-events.npz')['events']
        ...
        for method in ['D','G','G-exact']:
            for mode in modes:
                dest=ROOT/'outputs'/('panel' if args.panel else 'diagnostics')/session['session_id']/method/mode
        ...
        print(session['session_id'],readout_count(),flush=True)
