...
def execute(obs, dest, policy, removed=False, input_hash=None):
    ...
    try:
        ...
        with (dest/'stderr.txt').open('w') as err:
            ...
            response['broker_received_monotonic']=received
            response['broker_received_utc']=now()
            response['simulated_delivery_available_at']=response['simulation_observations_available_at']+max(0,received-receipt['received_monotonic'])
