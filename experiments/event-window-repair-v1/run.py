...
def run(split,config):
    ...
    if split=='heldout':
        ...
        for n,h in freeze['files'].items():
            assert digest((HERE/n).read_bytes())==h;assert digest(subprocess.check_output(['git','-C',str(HERE),'show','HEAD:experiments/event-window-repair-v1/'+n]))==h
    ...
    log=Log(d/'events.jsonl.gz');worker=Worker();cache=open_record(d/'cache.jsonl.gz','xt');waves=open_record(d/'wave_archive.jsonl.gz','xt');truthfile=open_record(d/'truth.jsonl.gz','xt');recipes=[];balance=[];invariance=[];nep=0;support_calls=0
    try:
      for g in groups:
        ...
        for e in episodes:
            ...
            support_calls+=pred['support_observation_calls']
