...
truth={r['episode']:r for r in map(json.loads,(P/'data/heldout/truth.jsonl').read_text().splitlines())};cfg=json.loads((P/'frozen_model.json').read_text());trials={(r['model'],r['group'],r['cell'],r['condition'],int(r['variant'])):r for r in csv.DictReader((P/'results/heldout/trials.csv').open())};error=0.;count=0;totals=defaultdict(lambda:[0,0])
for x in map(json.loads,(P/'data/heldout/cache.jsonl').read_text().splitlines()):
 t=truth[x['episode']];outputs={'streaming':x['operational']['forecasts'][cfg['selected']],'fixed':x['operational']['forecasts']['fixed'],'oracle-boundary':x['oracle_forecasts']}
