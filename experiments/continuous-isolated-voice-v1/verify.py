...
def main(split):
    d=HERE/'data'/split;cfg=json.loads((d/'config_used.json').read_text());truth={r['episode']:r for r in map(json.loads,(d/'truth.jsonl').read_text().splitlines())};cache=list(map(json.loads,(d/'cache.jsonl').read_text().splitlines()));waves={r['episode']:r for r in map(json.loads,(d/'wave_archive.jsonl').read_text().splitlines())};records=list(map(json.loads,(d/'events.jsonl').read_text().splitlines()));checks={};previous='0'*64;by=defaultdict(list)
    ...
    for x in cache:
        ...
        for name,ev in pred['events'].items():
            ...
            for e in ev:
                assert offset<=e['start_sample']<e['end_sample']<=e['available_sample']<=len(raw)//4;assert e['observation']['wave_sha256']==digest(raw[e['start_sample']*4:e['end_sample']*4]);maxeventdelay=max(maxeventdelay,(e['available_sample']-e['end_sample'])/16000)
        ...
        for front,fs in outputs.items():
            ...
            for name,f in fs.items():
                ...
                hh=h or [{'pitch_semitones':None}];p,point,atoms=reconstruct(hh,source,mc);err=max(abs(a-b) for a,b in zip(p,f['probabilities']));assert err<1e-12;maxerror=max(maxerror,err);assert point==f['point_pitch_semitones'] or (point is not None and f['point_pitch_semitones'] is not None and abs(point-f['point_pitch_semitones'])<1e-12);assert len(atoms)==len(f['lookup_evidence']);assert safe(measure(f,t))==scored[front][name];count+=1
    ...
    previous_pkg=HERE.parent/'fundamental-observation-repair-v1';assert (HERE/'accepted_adapter.py').read_bytes()==(previous_pkg/'model.py').read_bytes();assert (HERE/'reference_model.py').read_bytes()==(previous_pkg/'reference_model.py').read_bytes();assert cfg['adapter']==json.loads((previous_pkg/'frozen_model.json').read_text())['adapter'];assert cfg['reference']==json.loads((HERE/'reference_config.json').read_text())
