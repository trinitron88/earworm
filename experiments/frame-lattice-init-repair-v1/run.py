    ...
    try:
      for g in groups:
        ...
        for e in episodes:
            ...
            log.add('forecast_dispatch',dict(episode=eid));pred,pbytes=worker.rpc({'op':'forecast'});assert pred['evictions']==0 and pred['chunk_count']<=32
            ...
            if e['cell']=='detached' and e['condition']=='intact' and e['variant']==0:
                saved,_=worker.rpc({'op':'save'});worker.rpc({'op':'restore',**saved});restored,rbytes=worker.rpc({'op':'forecast'});assert rbytes==pbytes;v={'episode':eid,'same_forecast_bytes':True,'state_sha256':digest(base64.b64decode(saved['state_b64']))};restores.append(v);log.add('save_restore',v,True)
