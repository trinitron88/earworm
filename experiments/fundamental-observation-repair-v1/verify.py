...
def main(split):
    ...
    for x in cache:
        ...
        for name,f in x['forecast']['forecasts'].items():
            adapter,base=name.split('_',1);mcfg=dict(cfg['reference']);source=base
            if base.startswith('transition'):source='transition';mcfg['transition_order']=int(base[-1])
            elif base=='absolute':source='retrieval_absolute';mcfg['retrieval_absolute_order']=3
            elif base=='reference':source='retrieval_transposed'
            mh=h if adapter=='old' else [{**o,**o['repaired_observation']} for o in h]
            prob,point,atoms=reconstruct(mh,source,mcfg)
            diff=max(abs(a-b) for a,b in zip(prob,f['probabilities']));maxdiff=max(maxdiff,diff)
            assert diff<1e-12 and (point==f['point_pitch_semitones'] or (point is not None and f['point_pitch_semitones'] is not None and abs(point-f['point_pitch_semitones'])<1e-12));assert len(atoms)==len(f['lookup_evidence']);forecast_count+=1
        ...
        saved=next(r['scores'] for r in rs if r['kind']=='score')
        assert safe({n:measure(f,t) for n,f in x['forecast']['forecasts'].items()})==saved
