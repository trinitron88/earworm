...
def main(split):
    ...
    historical=json.loads((HERE/'prior_artifact_hashes.json').read_text());assert all(digest((HERE.parent.parent/n).read_bytes())==h for n,h in historical.items());assert sum(x['allocated_streams'] for x in json.loads((HERE/'run_ledger.json').read_text()))<=1000;assert sum(x.stat().st_size for x in HERE.rglob('*') if x.is_file())<2*1024**3
